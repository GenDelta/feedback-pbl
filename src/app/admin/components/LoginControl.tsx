...
import { getVisibilityState, updateVisibilityState } from "../actions/adminActions";

const LoginControl: React.FC = () => {
  const [isFacultyLoginActive, setIsFacultyLoginActive] = useState<boolean>(true);
  const [isStudentLoginActive, setIsStudentLoginActive] = useState<boolean>(true);
  const [isFacultyDashboardActive, setIsFacultyDashboardActive] = useState<boolean>(true);
  ...
  useEffect(() => {
    const fetchVisibilityStates = async () => {
      try {
        ...
        const facultyDashboardState = await getVisibilityState("facultyDashboard");
  ...
  const toggleFacultyLogin = async (status: boolean) => {
    try {
      setLoading(true);
      const success = await updateVisibilityState("facultyLogin", status ? 1 : 0);
      
      if (success) {
        setIsFacultyLoginActive(status);
        setUpdateStatus({
          success: true, 
          message: `Faculty login ${status ? 'activated' : 'deactivated'} successfully!`
        });
      } else {
        setUpdateStatus({
          success: false, 
  ...
  const toggleStudentLogin = async (status: boolean) => {
    try {
      setLoading(true);
      const success = await updateVisibilityState("studentLogin", status ? 1 : 0);
      
      if (success) {
        setIsStudentLoginActive(status);
        setUpdateStatus({
          success: true, 
          message: `Student login ${status ? 'activated' : 'deactivated'} successfully!`
        });
      } else {
        setUpdateStatus({
          success: false, 
  ...
  const toggleFacultyDashboard = async (status: boolean) => {
    try {
      setLoading(true);
      const success = await updateVisibilityState("facultyDashboard", status ? 1 : 0);
      
      if (success) {
        setIsFacultyDashboardActive(status);
        setUpdateStatus({
          success: true, 
          message: `Faculty dashboard ${status ? 'activated' : 'deactivated'} successfully!`
        });
      } else {
        setUpdateStatus({
          success: false, 
  ...
  return (
    <div>
      ...
      {/* Status message */}
      {updateStatus && (
        <div className={`p-4 mb-6 rounded ${updateStatus.success ? 'bg-green-600' : 'bg-red-600'}`}>
      ...
      {loading ? (
        ...
        <div className="space-y-12">
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            <h2 className="text-2xl font-semibold text-white mb-6">Student Login Control</h2>
            <div className="flex flex-col space-y-6">
              <div className="flex items-center justify-between">
                <p className="text-white text-lg">Current Status:</p>
                <span className={`px-3 py-1 rounded font-medium ${
                  isStudentLoginActive ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                }`}>
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            <h2 className="text-2xl font-semibold text-white mb-6">Faculty Login Control</h2>
            <div className="flex flex-col space-y-6">
              <div className="flex items-center justify-between">
                <p className="text-white text-lg">Current Status:</p>
                <span className={`px-3 py-1 rounded font-medium ${
                  isFacultyLoginActive ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                }`}>
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            <h2 className="text-2xl font-semibold text-white mb-6">Faculty Dashboard Control</h2>
            <div className="flex flex-col space-y-6">
              <div className="flex items-center justify-between">
                <p className="text-white text-lg">Current Status:</p>
                <span className={`px-3 py-1 rounded font-medium ${
                  isFacultyDashboardActive ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                }`}>
